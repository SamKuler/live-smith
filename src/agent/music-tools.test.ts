...
const service = choice({ id: "website", name: "Personal Suno", provider: "suno" as const });
...
const pluginTools = (services: readonly BuiltInIntegrationConnectionChoice[]) =>
  createBuiltInAudioToolsets({
    services,
...
test("custom music options are bounded and scoped to eligible connections", () => {
  const raw = { connectionId: service.id, prompt: "我的歌词", instrumental: false,
    durationSeconds: 120,
...
test("custom instrumental permits empty lyrics, while descriptions and vocal lyrics remain required", () => {
  const raw = { connectionId: service.id, prompt: "", instrumental: true, options: { mode: "custom" } };
  validateAudioServiceRequest(parse("generate_music", raw), [service]);
  assert.throws(() => parse("generate_music", { ...raw, instrumental: false }));
  assert.throws(() => parse("generate_music", { connectionId: service.id, prompt: "", instrumental: true }));
...
test("music browsing and editing have strict action-specific fields", () => {
  for (const input of [{ connectionId: service.id, query: "catalog" },
    { connectionId: service.id, query: "library", search: "piano", cursor: "opaque-next" },
    { connectionId: service.id, query: "persona", personaId: clipId }]) {
    const parsed = parse("inspect_music_service", input);
    validateAudioServiceRequest(parsed, [service]);
    assert.throws(() => validateAudioServiceRequest(parsed, [{ ...service, provider: "sunoapi" }]));
  }
  for (const input of [{ connectionId: service.id, query: "catalog", cursor: "extra" },
    { connectionId: service.id, query: "library", personaId: clipId },
    { connectionId: service.id, query: "persona" }, { connectionId: service.id, query: "publish" }]) {
    assert.throws(() => parse("inspect_music_service", input));
  }
  const extend = { connectionId: service.id, clipId, startSeconds: 30, prompt: "new verse", instrumental: false };
  ...
  validateAudioServiceRequest(parse("get_whole_song", { connectionId: service.id, clipId }), [service]);
